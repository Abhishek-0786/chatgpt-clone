...
async function loadDashboardData() {
    try {
        ...
        // Don't load mock data - let the loading spinners stay so user knows something is wrong
...
function updateDashboardStats(stats) {
    ...
    document.getElementById('totalEnergy').textContent = `${(stats.totalEnergy || 0).toFixed(2)} kWh`;
    document.getElementById('totalStations').textContent = stats.totalStations || 0;
    document.getElementById('totalChargers').textContent = stats.totalChargers || 0;
    document.getElementById('avgDuration').textContent = stats.avgDuration || '00:00';
    
    // Update changes
    document.getElementById('customersChange').textContent = `${stats.customersChange || 0}%`;
    document.getElementById('sessionsChange').textContent = `${stats.sessionsChange || 0}%`;
    document.getElementById('revenueChange').textContent = `${stats.revenueChange || 0}%`;
    document.getElementById('energyChange').textContent = `${stats.energyChange || 0}%`;
    ...
    if (stationsOnline === 0 && totalStations > 0) {
        document.getElementById('stationsStatus').textContent = `${stationsOffline} Offline`;
    } else {
        document.getElementById('stationsStatus').textContent = `${stationsOnline} Online`;
    }
    
    document.getElementById('chargersAvailable').textContent = `${stats.chargersAvailable || 0} Available`;
}
...
function updateTodayStats(todayStats) {
    document.getElementById('todaySessions').textContent = todayStats.sessions || 0;
    document.getElementById('todayRevenue').textContent = `₹${(todayStats.revenue || 0).toFixed(2)}`;
    document.getElementById('todayEnergy').textContent = `${(todayStats.energy || 0).toFixed(2)} kWh`;
    document.getElementById('todayCustomers').textContent = todayStats.newCustomers || 0;
}

// Update station status
function updateStationStatus(stationStatus) {
    document.getElementById('stationsOnlineCount').textContent = stationStatus.online || 0;
    document.getElementById('stationsOfflineCount').textContent = stationStatus.offline || 0;
    ...
    if (stationStatus.offlineStations && stationStatus.offlineStations.length > 0) {
        offlineList.innerHTML = `
            <div style="font-size: 13px; color: #64748b; margin-bottom: 10px; font-weight: 600;">Offline Stations:</div>
            ${stationStatus.offlineStations.map(s => `
                <div style="padding: 8px 12px; background: #fee2e2; border-radius: 6px; margin-bottom: 8px; font-size: 13px;">
        ...
        offlineList.innerHTML = '<div style="text-align: center; color: #94a3b8; padding: 20px;">All stations are online</div>';
    }
}

// Update charger status
function updateChargerStatus(chargerStatus) {
    document.getElementById('chargersAvailableCount').textContent = chargerStatus.available || 0;
    document.getElementById('chargersBusyCount').textContent = chargerStatus.busy || 0;
    document.getElementById('chargersUnavailableCount').textContent = chargerStatus.unavailable || 0;
    document.getElementById('chargersFaultedCount').textContent = chargerStatus.faulted || 0;
    ...
    if (chargerStatus.faultedChargers && chargerStatus.faultedChargers.length > 0) {
        faultedList.innerHTML = `
            <div style="font-size: 13px; color: #64748b; margin-bottom: 10px; font-weight: 600;">Faulted Chargers:</div>
            ${chargerStatus.faultedChargers.map(c => `
                <div style="padding: 8px 12px; background: #fee2e2; border-radius: 6px; margin-bottom: 8px; font-size: 13px;">
        ...
        faultedList.innerHTML = '<div style="text-align: center; color: #94a3b8; padding: 20px;">No faulted chargers</div>';
    }
}
