  ...
  try {
    ...
    switch (commandData.command) {
      case 'RemoteStartTransaction':
        routingKey = ROUTING_KEYS.COMMAND_REMOTE_START;
        break;
      case 'RemoteStopTransaction':
        routingKey = ROUTING_KEYS.COMMAND_REMOTE_STOP;
        break;
...
/**
 * Publish log to RabbitMQ queue (direct queue, not exchange)
 * Used for OCPP logs that need to be stored in database
 * @param {string} queueName - Queue name (e.g., 'ocpp.logs')
 * @param {Object} logData - Log data
 * @param {string} logData.deviceId - Device ID
 * @param {string} logData.messageType - Message type (BootNotification, etc.)
 * @param {Object} logData.payload - Message payload
 * @param {string} logData.direction - Direction (Incoming/Outgoing)
 * @param {string} logData.rawMessage - Raw message string
 * @param {Date} logData.timestamp - Timestamp
 * @returns {Promise<boolean>} - true if published successfully
 */
async function publishQueue(queueName, logData) {
  if (!isConnected()) {
    console.warn(`⚠️ RabbitMQ not connected, skipping queue publish to ${queueName}`);
    return false;
  }
  ...
  try {
    ...
    // Don't assert queue here - it's already declared during initialization
    // with proper settings (dead-letter-exchange, etc.)
    // Just use it directly to avoid PRECONDITION_FAILED errors

    const message = {
      deviceId: logData.deviceId,
      messageType: logData.messageType,
      payload: logData.payload || {},
      direction: logData.direction || 'Incoming',
    ...
    if (published) {
      console.log(`📤 Published log to queue ${queueName}: ${logData.messageType} from ${logData.deviceId}`);
      return true;
    } else {
      console.warn(`⚠️ Failed to publish log to queue ${queueName} (channel buffer full)`);
      return false;
    }
  } catch (error) {
    console.error(`❌ Error publishing log to queue ${queueName}:`, error.message);
    return false;
  }
}
