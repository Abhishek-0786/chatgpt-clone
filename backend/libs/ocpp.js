...
/**
 * Extract meter value from MeterValues log
 */
function extractMeterValue(meterValuesLog) {
  ...
  // Fallback: check if meterValue is directly in messageData with different structure
  const meterValueAlt = messageData.meterValue;
  if (meterValueAlt && Array.isArray(meterValueAlt) && meterValueAlt.length > 0) {
    const sampledValues = meterValueAlt[0].sampledValue;
